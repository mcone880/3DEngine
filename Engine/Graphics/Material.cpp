...
namespace MAC
{
	bool Material::Load(const std::string& filename, void* data)
	{
		...
		shader = engine->Get<MAC::ResourceSystem>()->Get<MAC::Program>(shader_name, engine);
		...
		{
			auto texture = engine->Get<MAC::ResourceSystem>()->Get<MAC::Texture>(name);	//<use the resource system and get Texture>(name);
