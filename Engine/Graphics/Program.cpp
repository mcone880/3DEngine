...
namespace MAC
{
	...
	bool Program::Load(const std::string& filename, void* data)
	{
		...
		bool success = MAC::json::Load(filename, document);
		if (!success)
		{
		...
		{
			auto vshader = engine->Get<MAC::ResourceSystem>()->Get<MAC::Shader>(vertex_shader, (void*)GL_VERTEX_SHADER);
			AddShader(vshader);
		}
		...
		JSON_READ(document, fragment_shader);
			...
			{
				auto fshader = engine->Get<MAC::ResourceSystem>()->Get<MAC::Shader>(fragment_shader, (void*)GL_FRAGMENT_SHADER);
				AddShader(fshader);
